	...
	class Consultas{
        ...
        function ConsultaPeriodo($_mes, $_anno){
			...
			$Informacion = 	$this->consulta_connect->PostgresSelectDistinctWhereGroupOrder("maestro.log_ciclo_muni_rutas", 
					"ciclo, sum(total) AS total, sum(total_certificar) as certificar, sum(total_entregadas) AS entregadas, sum(total_pendientes) AS pendientes", 
					"mes = ".$_mes." AND anno = ".$_anno, 
					"ciclo", 
					"ciclo"); 
			return json_encode($this->consulta_connect->QueryToJson($Informacion,["ciclo","total","certificar","entregadas","pendientes"],[null,null,null,null],false));
			$this->consulta_connect->ClosePostgres();
        }
        ...
        function ConsultaRutasCiclo($_mes, $_anno, $_ciclos){
			...
			return json_encode($this->consulta_connect->QueryToJson($Informacion,["ruta","inspector","total","certificar","leidas","pendientes","estado"],[null,null,null,null,null,null],false));
			$this->consulta_connect->ClosePostgres();
        }
        ...
        function ConsultaRutasEstado($_mes, $_anno, $_rutas){
			...
			$Informacion = 	$this->consulta_connect->PostgresSelectWhereOrder(	"maestro.log_ciclo_muni_ruta_cuentas", 
																				"ciclo||'-'||municipio||'-'||ruta AS ruta,cuenta,marca_contador||' '||numero_contador as medidor,nombre, direccion", 
																				"mes=".$_mes." AND anno=".$_anno." AND CAST(ciclo AS TEXT)||'-'||municipio||'-'||ruta IN (".$datos.") AND estado='P' AND certificar=1", 
																				"cuenta");			
		...
		function ConsultaLecturasTomadas($_mes, $_anno, $_inspectores){
			...
			$ClientesLeidos 	= $this->consulta_connect->QueryToJson($Informacion,
					["ruta","cuenta","medidor","nombre","direccion","fecha_entrega","fecha_registro","mensaje","id_inspector", "latitud","longitud", "distancia"],
        ...
        **/
        ...
        function consultarRutaCuentaDatos($mes, $anno, $ciclo, $municipio, $ruta){
        	$this->consulta_connect->OpenPostgres();
        	$Informacion = 	$this->consulta_connect->PostgresFunctionCamposTableOrder("fecha_toma,pocision,cuenta,medidor,nombre",
        																   			  "toma.consulta_pocision_gps_cuenta(".$mes.",".$anno.",".$ciclo.",'".$ruta."',".$municipio.") ",
        																   			  "fecha_toma"); 										
			$data = array();
			$arrayName = array();
			$i = 0;
